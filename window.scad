...
//
// A picture-frame border (outer rounded rectangle with a rectangular
// hole through the middle, like a real picture frame) mounted above a
// socket cylinder that is bored to take a 7 mm wooden dowel. The
// matching stand that holds the other end of the dowel is a separate
// part -- see dowel-stand.scad.
//
// The socket stays upright -- parallel to the Z axis, same as the
// dowel -- while only the frame tilts, hinged up and back off the
// socket's front rim by tilt_angle, so it leans over the socket
// without any part of it reaching down into the middle of the viewing
// opening. A tapered gusset (a smooth hull blend, like the root flare
// on a wine glass stem) fills the concave gap between the round
// socket and the flat underside of the frame, so the two parts are
// backed by a continuous wedge of solid material instead of meeting
// at a thin edge.
//
...
dowel_hole_depth = 20;    // how deep the dowel is expected to sit in the hole; must match dowel-stand.scad and mirror-spinner-stand.scad (socket_length below is defined from it), so change it there if you change it here
...
post_diameter   = 15;   // must match post_diameter in dowel-stand.scad
